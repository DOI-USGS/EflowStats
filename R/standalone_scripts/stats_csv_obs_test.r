####################################################################################################
# This script is used to parse data from WaterML2 for daily values
#
# The getXMLDV2Data function call data from the http://nwisvaws02.er.usgs.gov/ogc-swie/wml2/dv/sos?request=GetObservation&featureID=
#    website, which contains daily data.
#
# The following values are available in data frame Daily:
#   date     = date of measurement
#   value    = daily mean of measured property (usually discharge)
#####################################################################################################
...
a<-read.csv("sites_test_obs.txt",header=F,colClasses=c("character"))
a2<-length(a)
yv<-vector(length=a2)
...
for (i in 1:length(a)){
  #print(i)
  siteNumber<-a[i]
  x <- getXMLDV2Data(siteNumber,ParameterCd,StatCd,StartDate,EndDate,Interval,Latest)
...
output="allstats_obs.out"
write.table(statsout,file="allstats_obs.out",col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
