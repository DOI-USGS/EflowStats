...
SiteGoF <- function(Modeled,Gaged) {
  GandM = merge(Modeled[,c("date", "discharge")], 
                Gaged[,c("date", "discharge")],
				suffixes=c(".obs", ".mod"), by="date")
  GandM = GandM %>% arrange(date)

  if(nrow(GandM) < 2){ stop("No matching discharge days available") }
  ...
  ptile.res = GandM %>% group_by(ptile) %>% 
		        summarise(nsev = nse(discharge.obs, discharge.mod),
				...
				select(-ptile) %>% lapply(as.vector) %>% unlist
  
  GandM$month = strftime(GandM$date, "%m") %>% as.integer
  
  month.res = GandM %>% group_by(month) %>% 
                summarise(nsev = nse(discharge.obs, discharge.mod),
			    ...
			    select(-month) %>% lapply(as.vector) %>% unlist
